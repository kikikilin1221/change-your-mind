...
// --- 四角形の中心と対象座標から「辺（ふち）」の交点を求める計算関数 ---
...
function FlowEditor() {
  ...
  }, [nodes, edges, currentLevel, currentLabel, levelData]);
  ...
  const flowNodes = useMemo(() => {
    const centerNode = { 
      ...
      data: { label: '＋' }, 
      style: { width: 20, height: 20, minWidth: 20, minHeight: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444', fontSize: '14px', fontWeight: 'bold', background: 'rgba(255,255,255,0.7)', borderRadius: '50%', border: '2px solid #ef4444', zIndex: -1000, pointerEvents: 'none', padding: 0 } 
    };
            ...
            {n.data.previewVisible && !n.data.isShape && !n.data.isImage && (() => {
              ...
              return (
                <>
                  ...
                  {/* 吹き出し本体（nodragクラスによりReact Flowのドラッグをキャンセルし、独立移動） */}
                  <div 
                    className="nodrag"
                    onMouseDown={(e) => {
                      e.stopPropagation();
                      previewDragRef.current = { id: n.id, startX: e.clientX, startY: e.clientY, initX: offsetX, initY: offsetY, moved: false };
                    }}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!previewDragRef.current?.moved) enterLevel(n.id, String(n.data.content));
                    }}
                    style={{ position:'absolute', left: offsetX, top: offsetY, width: `${w2}px`, height: `${h2}px`, backgroundColor:`rgba(255,255,255,${n.data.previewStyle?.opacity || 0.7})`, borderRadius: '12px', border: '2px solid #ccc', zIndex: -1, cursor: 'grab', overflow: 'hidden', boxShadow: '0 8px 12px rgba(0,0,0,0.1)' }}
                  >
                    {levelData[n.id] && levelData[n.id].nodes && levelData[n.id].nodes.length > 0 ? (
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'row' }}>
      ...
      <div style={{ width: '220px', backgroundColor: '#f8f9fa', borderRight: '1px solid #ddd', display: 'flex', flexDirection: 'column', padding: '15px', zIndex: 10 }}>
        <h2 style={{ fontSize: '14px', color: '#666', marginBottom: '15px' }}>ファイル一覧</h2>
        <button onClick={createNewFile} style={{ padding: '8px', marginBottom: '20px', backgroundColor: '#fff', border: '1px solid #ddd', cursor: 'pointer', fontWeight: 'bold' }}>＋ 新規ノート</button>
        <div style={{ flexGrow: 1, overflowY: 'auto' }}>
          {Object.entries(files).map(([id, f]: [string, any]) => (
            <div key={id} onClick={() => loadFile(id)} style={{ padding: '8px', marginBottom: '5px', borderRadius: '4px', cursor: 'pointer', backgroundColor: activeFileId === id ? '#e7f1ff' : 'transparent', border: activeFileId === id ? '1px solid #3b82f6' : '1px solid transparent', fontSize: '13px' }}>
              {f.name} <span onClick={(e) => { e.stopPropagation(); deleteFile(id); }} style={{ float: 'right', color: '#ccc' }}>×</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', backgroundColor: levelData[currentLevel]?.bgColor || '#ffffff', transition: 'background-color 0.3s' }}>
        <div style={{ padding: '10px', backgroundColor: 'rgba(255,255,255,0.8)', borderBottom: '1px solid #eee', textAlign: 'center', fontWeight: 'bold', zIndex: 100 }}>階層: {currentLabel} ({files[activeFileId]?.name})</div>
        <div style={{ flexGrow: 1, position: 'relative' }}>
          <ReactFlow 
            nodes={flowNodes} edges={edges} edgeTypes={edgeTypes} 
          ...
          {selectedNode && (
            <div style={{ position:'absolute', right:0, top:0, bottom:0, width:'340px', borderLeft:'1px solid #ddd', padding:'20px', backgroundColor:'#fff', zIndex:1000, overflowY: 'auto', boxShadow: '-4px 0 10px rgba(0,0,0,0.05)' }}>
              ...
              <h3 style={{fontSize:'16px'}}>{selectedNode.data.isImage ? '画像編集' : selectedNode.data.isShape ? '図形設定' : '項目設定'}</h3>
              
              <div style={{ display:'flex', gap:'5px', marginBottom:'15px', marginTop:'10px' }}>
                <button onClick={() => { 
                  setNodes(nds => {
                    const maxZ = Math.max(0, ...nds.map(n => Number(n.zIndex) || 0));
                    return nds.map(n => n.id === selectedNodeId ? {...n, zIndex: maxZ + 1} : n);
                  }); 
                }} style={{flex:1, padding:'8px', fontSize:'12px', fontWeight: 'bold', background: '#f0f0f0', border: '1px solid #ccc', cursor: 'pointer'}}>↑ 最前面へ</button>
                <button onClick={() => { 
                  setNodes(nds => {
                    const minZ = Math.min(0, ...nds.map(n => Number(n.zIndex) || 0));
                    return nds.map(n => n.id === selectedNodeId ? {...n, zIndex: minZ - 1} : n);
                  }); 
                }} style={{flex:1, padding:'8px', fontSize:'12px', fontWeight: 'bold', background: '#f0f0f0', border: '1px solid #ccc', cursor: 'pointer'}}>↓ 最背面へ</button>
              </div>

              {selectedNode.data.isImage && (
                <div style={{ marginBottom: '20px', padding: '10px', border: '1px solid #eee', borderRadius: '8px' }}>
                  <label style={{fontSize: '11px', fontWeight: 'bold'}}>中身のズーム / 位置</label>
              ...
              {selectedNode.data.isShape && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px', marginBottom: '15px' }}>
                  <button onClick={() => {
                    const size = Math.max(Number(selectedNode.style?.width) || 150, Number(selectedNode.style?.height) || 150);
                    updateNode({ shapeType: 'rect', keepRatio: true }, { borderRadius: '0px', width: size, height: size });
                  }} style={{ padding: '8px', cursor: 'pointer', background: selectedNode.data.shapeType === 'rect' && selectedNode.data.keepRatio ? '#ddd' : '#fff', border: '1px solid #ccc' }}>■ 正方形</button>
                  ...
                  <button onClick={() => {
                    const size = Math.max(Number(selectedNode.style?.width) || 150, Number(selectedNode.style?.height) || 150);
                    updateNode({ shapeType: 'circ', keepRatio: true }, { borderRadius: '50%', width: size, height: size });
                  }} style={{ padding: '8px', cursor: 'pointer', background: selectedNode.data.shapeType === 'circ' && selectedNode.data.keepRatio ? '#ddd' : '#fff', border: '1px solid #ccc' }}>● 正円</button>
              ...
              {!selectedNode.data.isImage && <textarea value={String(selectedNode.data.content || '')} onChange={(e) => updateNode({ content: e.target.value })} style={{ width:'100%', height:'120px', marginBottom: '10px', padding: '8px', border: '1px solid #ddd' }} />}
              
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'5px', marginBottom:'10px' }}>
                <button onClick={() => updateNode({}, { fontFamily: 'serif' })} style={{cursor:'pointer', border:'1px solid #ccc'}}>明朝</button>
                <button onClick={() => updateNode({}, { fontFamily: 'sans-serif' })} style={{cursor:'pointer', border:'1px solid #ccc'}}>ゴシック</button>
                <button onClick={() => updateNode({}, { fontWeight: selectedNode.style?.fontWeight === 'bold' ? 'normal' : 'bold' })} style={{ cursor:'pointer', border:'1px solid #ccc', background: selectedNode.style?.fontWeight === 'bold' ? '#ddd' : '#f0f0f0' }}>太字</button>
                <button onClick={() => updateNode({}, { textDecoration: selectedNode.style?.textDecoration?.includes('double') ? 'none' : 'line-through double' })} style={{ cursor:'pointer', border:'1px solid #ccc', background: selectedNode.style?.textDecoration?.includes('double') ? '#ddd' : '#f0f0f0' }}>二重線</button>
              </div>
              <div style={{ display:'flex', gap:'5px', marginBottom:'15px' }}>
                <button onClick={() => updateNode({}, { textAlign: 'left' })} style={{flex:1, cursor:'pointer', border:'1px solid #ccc'}}>左揃え</button>
                <button onClick={() => updateNode({}, { textAlign: 'center' })} style={{flex:1, cursor:'pointer', border:'1px solid #ccc'}}>中央揃え</button>
                <button onClick={() => updateNode({}, { textAlign: 'right' })} style={{flex:1, cursor:'pointer', border:'1px solid #ccc'}}>右揃え</button>
              </div>

              <label style={{fontSize:'11px', fontWeight: 'bold'}}>文字サイズ</label>
              <div style={{display:'flex', alignItems:'center', gap:'5px', marginBottom: '10px'}}>
                ...
                <input type="number" min="10" max="250" value={parseInt(String(selectedNode.style?.fontSize || 18))} onChange={(e) => updateNode({}, { fontSize: `${e.target.value}px` })} style={{width:'60px', padding:'2px', border:'1px solid #ccc', borderRadius:'4px'}} /> px
              </div>
              <label style={{fontSize:'11px', fontWeight: 'bold'}}>透明度</label>
              <input type="range" min="0.1" max="1" step="0.1" value={Number(selectedNode.style?.opacity ?? 1)} onChange={(e) => updateNode({}, { opacity: parseFloat(e.target.value) })} style={{width:'100%', marginBottom:'10px'}} />

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '5px', marginTop: '10px' }}>
                {QUICK_TEXT_COLORS.map(c => <button key={c} onClick={() => updateNode({}, { color: c })} style={{ width:'25px', height:'25px', backgroundColor:c, border: '1px solid #eee', cursor: 'pointer' }} />)}
                <input type="color" onChange={(e) => updateNode({}, { color: e.target.value })} style={{width:'25px', height:'25px', cursor: 'pointer'}} title="文字色" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '5px', marginTop: '5px' }}>
                {PASTEL_COLORS.map(c => <button key={c} onClick={() => updateNode({}, { backgroundColor: c })} style={{ width:'25px', height:'25px', backgroundColor:c, border: '1px solid #eee', cursor: 'pointer' }} />)}
                <input type="color" value={String(selectedNode.style?.backgroundColor || '#ffffff')} onChange={(e) => updateNode({}, { backgroundColor: e.target.value })} style={{width:'25px', height:'25px', cursor: 'pointer'}} title="背景色" />
              </div>

              {!selectedNode.data.isShape && !selectedNode.data.isImage && (
                <div style={{marginTop: '20px', borderTop: '1px solid #eee', paddingTop: '10px'}}>
                  <label style={{fontSize:'11px', fontWeight:'bold'}}>吹き出し(プレビュー枠) 設定:</label>
                  
                  <button onClick={() => updateNode({ previewVisible: !selectedNode.data.previewVisible })} style={{ width: '100%', padding: '8px', marginTop: '5px', marginBottom: '10px', background: selectedNode.data.previewVisible ? '#3b82f6' : '#fff', color: selectedNode.data.previewVisible ? '#fff' : '#333', border: '1px solid #ccc', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
                    {selectedNode.data.previewVisible ? '吹き出しを消す' : '吹き出しを追加'}
                  </button>

                  {selectedNode.data.previewVisible && (
                    <>
                      <p style={{fontSize: '10px', color: '#666', marginBottom: '5px'}}>※位置は吹き出しを直接ドラッグして動かせます</p>
              ...
              <button onClick={() => { setNodes(nds => nds.filter(n => n.id !== selectedNodeId)); setSelectedNodeId(null); }} style={{ width:'100%', marginTop:'20px', color: 'red', border: '1px solid red', padding: '8px', cursor: 'pointer', fontWeight: 'bold' }}>削除</button>
            </div>
          )}
          ...
          {selectedEdge && (
            <div style={{ position:'absolute', right:0, top:0, bottom:0, width:'320px', borderLeft:'1px solid #ddd', padding:'20px', backgroundColor:'#fff', zIndex:1000 }}>
              ...
              <div style={{ display:'flex', gap:'5px', marginBottom:'20px' }}>
                {[2, 6, 12].map(w => <button key={w} onClick={() => setEdges(eds => eds.map(e => e.id === selectedEdge.id ? { ...e, style: { ...e.style, strokeWidth: w } } : e))} style={{flex:1, padding:'10px', cursor: 'pointer', border: '1px solid #ccc'}}>{w === 2 ? '細' : w === 6 ? '中' : '太'}</button>)}
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                {[{l:'普通',c:{}},{l:'片矢印',c:{arrow:true}},{l:'二重片矢印',c:{double:true,arrow:true}},{l:'両矢印',c:{both:true}},{l:'二重両矢印',c:{double:true,both:true}},{l:'論理和(∧)',c:{label:'∧'}},{l:'論理積(∨)',c:{label:'∨'}}].map(item => (
                  <button key={item.l} onClick={() => updateEdgeDesign(item.c)} style={{padding:'8px', border: '1px solid #ccc', background: '#f9f9f9', cursor: 'pointer'}}>{item.l}</button>
                ))}
              </div>
              <button onClick={() => { setEdges(eds => eds.filter(e => e.id !== selectedEdge.id)); setSelectedEdge(null); }} style={{ width:'100%', marginTop:'30px', color: 'red', border: '1px solid red', background: 'none', padding: '8px', cursor: 'pointer', fontWeight: 'bold' }}>線を削除</button>
            </div>
          )}
        </div>
        <div style={{ padding: '15px', backgroundColor: 'rgba(255,255,255,0.9)', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '15px', zIndex: 1001 }}>
          <button onClick={goBack} disabled={isRoot} style={{ padding: '10px 15px', borderRadius:'8px', opacity: isRoot ? 0.3 : 1, cursor: isRoot ? 'default' : 'pointer', backgroundColor: '#eee', border: '1px solid #ccc' }}>← 1つ前</button>
          <button onClick={goTop} disabled={isRoot} style={{ padding: '10px 15px', borderRadius:'8px', opacity: isRoot ? 0.3 : 1, cursor: isRoot ? 'default' : 'pointer', fontWeight: 'bold', backgroundColor: '#eee', border: '1px solid #ccc' }}>TOP層へ</button>
          
          <div style={{ width: '1px', height: '30px', backgroundColor: '#ddd', margin: '0 5px' }} />
          
          <button onClick={() => setViewport({ x: 0, y: 0, zoom: 1 }, { duration: 800 })} style={{ padding: '10px 15px', borderRadius: '8px', border: '2px solid #3b82f6', cursor: 'pointer', background: '#fff' }}>中央</button>
          <button onClick={() => addNode('text')} style={{ padding: '10px 20px', backgroundColor: '#3b82f6', color: '#fff', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', border: 'none' }}>＋ テキスト</button>
          <button onClick={() => addNode('image')} style={{ padding: '10px 20px', backgroundColor: '#10b981', color: '#fff', borderRadius: '8px', cursor: 'pointer', border: 'none' }}>📷 画像</button>
          <button onClick={() => addNode('shape')} style={{ padding: '10px 20px', backgroundColor: '#f59e0b', color: '#fff', borderRadius: '8px', cursor: 'pointer', border: 'none' }}>square 図形</button>
          
          <div style={{ width: '1px', height: '30px', backgroundColor: '#ddd', margin: '0 5px' }} />
          
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: 'bold', color: '#666' }}>
            階層背景色: 
